...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-b", "--batch_process_dir", required=True, help="Directory of files to process"
    )
    parser.add_argument(
        "-o", "--output_path", required=True, help="Path to save the output JSON file"
    )
    parser.add_argument(
        "-t", "--topic", required=True, help="Topic of the script your going to analyze"
    )
    ...
    assert os.path.exists(
        args.batch_process_dir
    ), f"batch_process_dir: {args.batch_process_dir} not exists"
    assert not os.path.isdir(
        args.output_path
    ), "The specified output path is a directory. Please provide a file path."
