...
from config.constants import WIKIPEDIA_API_CONFIG, TOPICS_JSON
from src.utils import read_yaml, load_json
...
def main():
    wikipedia_config = read_yaml(WIKIPEDIA_API_CONFIG)
    wiki_access_token = wikipedia_config["access_token"]
    ...
    #     predicted_quality = get_predicted_quality(topic)
    #     if predicted_quality is None:
    #         logger.error(f'Fail to include "{topic}"')
    #         continue
    #     data['topic'].append(topic)
    #     data['url'].append(f'https://en.wikipedia.org/wiki/{topic.replace(" ", "_")}')
